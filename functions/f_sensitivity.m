function [dydp,dyND] = f_sensitivity(y,idx,parameterReached,xInitial,parameters,tExp,par2names, stateNames)

timeFrames = linspace(tExp(1),tExp(end));
nneg = 1:length(xInitial);
options=odeset('RelTol',1e-4,'NonNegative',nneg);
% options=odeset('RelTol',1e-4);

% sensitvity of outputs
idY = zeros(1,length(stateNames));
for i = 1:length(stateNames)
    idY(i) = find(strcmp(parameters.compoundAbb,stateNames{i}));
end 
...
yND = repmat(mean(y),size(timeFrames,2)*2,1);   % Matrix containing the vector of output mean values repeated t times.
...
for i=1:length(idx)                      % Building the diferential sensibility matrix (one column for each parameter)
    for j=1:2
        ...
        if j==1
            [~,y1] = ode15s(@f_mass_balances,timeFrames,xInitialUnc(1,:),options,parameters);
            [~,y2] = ode15s(@f_mass_balances,timeFrames,xInitialUnc(2,:),options,parameters);
            yPlus=[y2; y1];
        end
        if j==2
            [~,y1] = ode15s(@f_mass_balances,timeFrames,xInitialUnc(1,:),options,parameters);
