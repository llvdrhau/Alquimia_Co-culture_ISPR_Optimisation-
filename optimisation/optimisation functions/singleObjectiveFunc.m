function [costFunc,costFunc2,parameters,t,y,reservoir] = singleObjectiveFunc(decisionVar,parameters,xInitial,caso)
...
%    This script calculates the productivity or yield of proprionate
%    according to input of "caso" for single obj optimisation
...
switch caso
    case 'yield'
        costFunc2 = reservoir(end,pos_pro)/t(end); % PROductivity
        yieldActual = -propMade/gluAdded ; % YIELD gCOD/gCOD
        
        if  costFunc2 < 0.6
            costFunc  = max(yieldActual,-0.7) + 999;  % so simulation makes sure all propionate is extracted
        else
            costFunc  = max(yieldActual,-0.7);
        end
        
    case 'productivity'
        
        yieldActual = -propMade/gluAdded ; % YIELD gCOD/gCOD
        costFunc2  = max(yieldActual,-0.7);  % so simulation doesn't stretch out
        
        if y(end,pos_pro) > 0.05 
            costFunc  = -reservoir(end,pos_pro)/t(end) + 999;  % so simulation makes sure all propionate is extracted
        else
            costFunc  = -reservoir(end,pos_pro)/t(end);
        end
